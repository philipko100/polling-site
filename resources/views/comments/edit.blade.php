...
<div class="page-content">
                    ...
                    @else
                        @if(!Auth::guest())
                            @if(Auth::user()->id == $comment->user_id)
                                    ...
                                    {!!Form::open(['action'=>['CommentsController@update', $comment->id], 'method'=>'PUT', 'class'=>'pull-right','style'=>'height:100px'])!!}
                                        {{Form::textarea('body', $user_comment->body, ['class'=>'form-control', 'placeholder'=>'Enter new comment here',
                                         'autofocus', 'style'=>'width:100%; height:90%'])}}
                                        {{Form::submit('Edit Comment', ['class'=>'btn btn-secondary btn-sm'])}}
                                    {!!Form::close() !!}
    ...
    {!!Form::open(['action'=>['CommentsController@store'], 'method'=>'POST', 'class'=>'pull-right'])!!}
        {{Form::textarea('body', '', ['class'=>'form-control', 'placeholder'=>'Comment on the review here',
